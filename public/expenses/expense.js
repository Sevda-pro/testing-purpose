...
document.querySelector("#pagebutton").addEventListener("click", (e) => {
	let val = document.querySelector("#myselect").value;
...
const getcurrPage = async function (currpage) {
	try {
		...
		const res = await axios.get(`http://51.20.4.199/expense/pagination?currpage=${currpage}&expPerPage=${noofexpperpage}`, { headers: { Authorization: token } });
...
async function paymentProcess(e) {
	try {
		...
		const response = await axios.get("http://51.20.4.199/purchase/premiummembership", { headers: { Authorization: token } });
		...
		var options = {
			...
			handler: async function (response) {
				const res = await axios.post(
					"http://51.20.4.199/purchase/updatetransactionstatus/success",
					{
		...
		rzp1.on("payment.failed", async function (response) {
			...
			await axios.post(
				"http://51.20.4.199/purchase/updatetransactionstatus/failed",
				{
...
async function add_to_database(obj) {
	try {
		const token = localStorage.getItem("token");
		await axios.post("http://51.20.4.199/expense", obj, { headers: { Authorization: token } });
		getcurrPage(localStorage.getItem("currPageOnScreen"));
	} catch (error) {
...
async function get_from_database() { 
	try {
		...
		let arr = await axios.get("http://51.20.4.199/expense", { headers: { Authorization: token } });
		return arr.data;
	} catch (error) {
		console.log(error);
	}
...
async function showleaderboard() {
	try {
		...
		let token = localStorage.getItem("token");
		let newarr = await axios.get("http://51.20.4.199/premium/leaderboard", { headers: { Authorization: token } });
		let arr = newarr.data;
...
async function delete_from_database(id) {
	try {
		let token = localStorage.getItem("token");
		await axios.delete(`http://51.20.4.199/expense/${id}`, { headers: { Authorization: token } });
		getcurrPage(1);
	} catch (error) {
		console.log(error);
		let errorDiv = document.createElement("div");
...
async function download() {
	try {
		let token = localStorage.getItem("token");
		let res = await axios.get("http://51.20.4.199/expense/download", { headers: { Authorization: token } });
...
async function show_fileURL_list() {
	try {
		let token = localStorage.getItem("token");
		let lis = await axios.get("http://51.20.4.199/expense/allfiles", { headers: { Authorization: token } });
		let arr = lis.data;
